...
PORT = 'COM5'  # 아두이노 열결후 몇번 포트로 연결되는지 확인하고 적어야댐
BaudRate = 9600
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        # Serial 읽기. readable()을 이용해 serial을 받지 않아도 프레임을 돌리게 하는 게 좋음
        cur = ARD.readline()
        ...
        if cur == "1" or cnt != 0:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    # Print results
                    # det[:, -1]???? 무슨 의미지??
                    for c in det[:, -1].unique():  # 한 객체가 여러개 판별된 경우 문자로는 한번만 출력하기 위해 unique사용
